...
# 4. Begin to examine (each team will begin a new script at this point to be submitted)
#######################################
...
# a. food security
# NOTE: I believe you really want df3yearwide, but this is not yet prepped for merging
# also need to identify key variables among 20 or so provided
...
df <- left_join(disp_conflict, food, by = c("iso", "year")) %>% 
  rename(gdo_fao = gdp) # gdp exists as variable in multiple sources; rename to identify source
...
rm(food, gini, polity, wbd, wdi, pts)
...
aglist <- c("agri_percent_land_area", "agri_forestry_fishing_percent_GDP", "imports_percent_GDP")
summary(df[,aglist])
# aggregation plot
...
# c. food security (Jackie and Stearns)
# fslist <- c("") # fill this in with at least 3 relevant variables and begin to examine


# For each variable -- submit a script (or markdown file) and commentary 
#   describing when and where the variables exhibit missingness
#   whether, given the when and where (or codebooks), there are plausible explanations for the missingness
